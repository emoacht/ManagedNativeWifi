	...
	public static IEnumerable<string> GetAvailableNetworkSsids()
	{
		...
		try
		{
			if (WlanOpenHandle(
				...
				out clientHandle) is not ERROR_SUCCESS)
				yield break;

			if (WlanEnumInterfaces(
				clientHandle,
				IntPtr.Zero,
				out interfaceList) is not ERROR_SUCCESS)
				yield break;
			...
			foreach (var interfaceInfo in interfaceInfoList.InterfaceInfo)
			{
				if (WlanGetAvailableNetworkList(
					...
					IntPtr.Zero,
					out availableNetworkList) is not ERROR_SUCCESS)
					continue;
		...
		try
		{
			...
			foreach (var interfaceInfo in interfaceInfoList.InterfaceInfo)
			{
				if (WlanQueryInterface(
					...
					out uint dataSize,
					ref queryData,
					IntPtr.Zero) is not ERROR_SUCCESS) // If not connected to a network, ERROR_INVALID_STATE will be returned.
					continue;
