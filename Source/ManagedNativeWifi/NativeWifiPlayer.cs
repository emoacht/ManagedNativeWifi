...
namespace ManagedNativeWifi
{
	...
	public class NativeWifiPlayer : IDisposable
	{
		...
		public NativeWifiPlayer()
		{
			...
			_client.Register(WLAN_NOTIFICATION_SOURCES.WLAN_NOTIFICATION_SOURCE_ACM
			                 | WLAN_NOTIFICATION_SOURCES.WLAN_NOTIFICATION_SOURCE_MSM);
		...
		private void OnNotificationReceived(object sender, WLAN_NOTIFICATION_DATA e)
		{
			...
			{
				case WLAN_NOTIFICATION_SOURCES.WLAN_NOTIFICATION_SOURCE_ACM:
				...
				case WLAN_NOTIFICATION_SOURCES.WLAN_NOTIFICATION_SOURCE_MSM:
		...
		{
			_phyTypesCache ??= [];
			if (!_phyTypesCache.TryGetValue(interfaceId, out var phyTypes))
			{
...
#endif
					.ToArray();
				phyTypes = Array.AsReadOnly(dot11PhyTypes.Select(PhyTypeConverter.Convert).ToArray());
		...
		/// <summary>
		/// Sets (add or overwrite) the user data (credentials) for a specified wireless profile.
		/// </summary>
