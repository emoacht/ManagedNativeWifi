...
namespace ManagedNativeWifi
{
	...
	public class NativeWifi
	{
		...
		internal static async Task<IEnumerable<Guid>> ScanNetworksAsync(Base.WlanNotificationClient client, ScanMode mode, IEnumerable<Guid> interfaceIds, NetworkIdentifier ssid, TimeSpan timeout, CancellationToken cancellationToken)
		{
			...
			if ((mode is ScanMode.OnlySpecified) && (specifiedIds is null or { Length: <= 0 }))
			...
			var counter = new ScanCounter(() => Task.Run(() => tcs.TrySetResult(true), cancellationToken), targetIds);

			container.Content.Register(WLAN_NOTIFICATION_SOURCES.WLAN_NOTIFICATION_SOURCE_ACM);
			...
			//Todo: logic to handle null in targetIds?
		...
		{
			return EnumerateAvailableNetworks(null as Base.WlanClient);
		}
		...
		/// Other if the interface is found or the function failed.
		/// </para>
		...
		private static IEnumerable<AvailableNetworkInfo> EnumerateAvailableNetworks(IEnumerable<WLAN_AVAILABLE_NETWORK> availableNetworks)
		{
			...
			{
				if (!BssTypeConverter.TryConvert(availableNetwork.dot11BssType, out var bssType) ||
				    !AuthenticationAlgorithmConverter.TryConvert(availableNetwork.dot11DefaultAuthAlgorithm, out var authenticationAlgorithm) ||
				    !CipherAlgorithmConverter.TryConvert(availableNetwork.dot11DefaultCipherAlgorithm, out var cipherAlgorithm))
					continue;
				...
				yield return new AvailableNetworkInfo(
					...
					profileName: availableNetwork.strProfileName.ToString(),
		...
		private static IEnumerable<AvailableNetworkGroupPack> EnumerateAvailableNetworkGroups(Base.WlanClient client, InterfaceInfo interfaceInfo)
		{
			foreach (var availableNetwork in Base.GetAvailableNetworkList(client.Handle, interfaceInfo.Id).list)
			{
				...
				var bssNetworks = Base.GetNetworkBssEntryList(client.Handle, interfaceInfo.Id,
					...
					.Select(x => TryConvertBssNetwork(x, out var bssNetworkInfo) ? new BssNetworkPack(interfaceInfo, bssNetworkInfo) : null)
					.Where(x => x is not null);

				//todo: check if any bssNetworks found?
		...
		{
			return EnumerateBssNetworks(null as Base.WlanClient);
		}
		...
		internal static (ActionResult result, IEnumerable<BssNetworkInfo> list) EnumerateBssNetworks(Base.WlanClient client, Guid interfaceId)
		{
			...
			return result is not ActionResult.Success
				? (result, null)
				: (ActionResult.Success, EnumerateBssNetworks(list));
		}
		...
		private static IEnumerable<BssNetworkInfo> EnumerateBssNetworks(WLAN_BSS_ENTRY[] bssEntries)
		{
			...
			{
				if (TryConvertBssNetwork(bssEntry, out var bssNetworkInfo))
		...
		{
			if (BssTypeConverter.TryConvert(bssEntry.dot11BssType, out var bssType))
			{
				TryDetectBandChannel(bssEntry.ulChCenterFrequency, out var band, out var channel);
		...
		internal static (ActionResult result, CurrentConnectionInfo value) GetCurrentConnection(Base.WlanClient client, Guid interfaceId)
		{
			...
			if (!BssTypeConverter.TryConvert(value.wlanAssociationAttributes.dot11BssType, out var bssType) ||
			    !AuthenticationAlgorithmConverter.TryConvert(value.wlanSecurityAttributes.dot11AuthAlgorithm, out var authenticationAlgorithm) ||
			    !CipherAlgorithmConverter.TryConvert(value.wlanSecurityAttributes.dot11CipherAlgorithm, out var cipherAlgorithm))
			...
			return (ActionResult.Success, new CurrentConnectionInfo(
				...
				profileName: value.strProfileName.ToString(),
		...
		internal static (ActionResult result, RealtimeConnectionQualityInfo value) GetRealtimeConnectionQuality(Base.WlanClient client, Guid interfaceId)
		{
			...
			if (result is not ActionResult.Success)
				return (result, null);
			if (value is not { } connectionQuality)
				return (ActionResult.OtherError, null);
			
			var links = connectionQuality.linksInfo
				.AsSpan((int)connectionQuality.ulNumLinks).ToArray()
				.Select(link => new RealtimeConnectionQualityLinkInfo(
			...
			return (ActionResult.Success, new RealtimeConnectionQualityInfo(
				phyType: PhyTypeConverter.Convert(connectionQuality.dot11PhyType),
				linkQuality: (int)connectionQuality.ulLinkQuality,
				rxRate: (int)connectionQuality.ulRxRate,
				txRate: (int)connectionQuality.ulTxRate,
				isMultiLinkOperation: connectionQuality.bIsMLOConnection,
				links: links));
		}
		...
		internal static IEnumerable<string> EnumerateProfileNames(Base.WlanClient client)
		{
			...
			foreach (var interfaceInfo in Base.GetInterfaceInfoList(container.Content.Handle))
			{
				foreach (var profileInfo in Base.GetProfileInfoList(container.Content.Handle, interfaceInfo.InterfaceGuid))
					yield return profileInfo.strProfileName.ToString();
			}
		}
		...
		internal static IEnumerable<ProfilePack> EnumerateProfiles(Base.WlanClient client)
		{
			...
			foreach (var interfaceInfo in EnumerateInterfaces(container.Content))
			{
				...
				{
					var profileXml = Base.GetProfile(container.Content.Handle, interfaceInfo.Id,
						profileInfo.strProfileName.ToString(), out var profileTypeFlag);
					if (string.IsNullOrWhiteSpace(profileXml))
						continue;

					if (!ProfileTypeConverter.TryConvert(profileTypeFlag, out var profileType))
						continue;

					yield return new ProfilePack(
						interfaceInfo: interfaceInfo,
						name: profileInfo.strProfileName.ToString(),
				...
				foreach (var profileInfo in Base.GetProfileInfoList(container.Content.Handle, interfaceConnectionInfo.Id))
				{
					...
					if (isConnected)
					{
						...
						isConnected = (result is ActionResult.Success)
						              && string.Equals(profileInfo.strProfileName.ToString(), value.strProfileName.ToString(), StringComparison.Ordinal);
					}

					var profileXml = Base.GetProfile(container.Content.Handle, interfaceConnectionInfo.Id, profileInfo.strProfileName.ToString(), out uint profileTypeFlag);
					if (string.IsNullOrWhiteSpace(profileXml))
						continue;
					...
					var availableNetworkGroup = availableNetworkGroups.FirstOrDefault(x => string.Equals(x.ProfileName, profileInfo.strProfileName.ToString(), StringComparison.Ordinal));
		...
		{
			const uint WLAN_REASON_CODE_SUCCESS = 0;
			...
			container.Content.NotificationReceived += (_, data) =>
			{
				...
				if (connectionNotificationData.strProfileName.ToString() != profileName)
					return;

				switch ((WLAN_NOTIFICATION_ACM)data.NotificationCode)
				{
					case WLAN_NOTIFICATION_ACM.wlan_notification_acm_connection_complete:
						var isSuccess = (connectionNotificationData.wlanReasonCode is WLAN_REASON_CODE_SUCCESS);
						Task.Run(() => tcs.TrySetResult(isSuccess), cancellationToken);
						break;
					case WLAN_NOTIFICATION_ACM.wlan_notification_acm_connection_attempt_fail:
						...
						Task.Run(() => tcs.TrySetResult(false), cancellationToken);
						break;
				}
			};

			var result = bssid is not null
				? Base.Connect(container.Content.Handle, interfaceId, profileName, BssTypeConverter.ConvertBack(bssType), dot11MacAddress)
				: Base.Connect(container.Content.Handle, interfaceId, profileName, BssTypeConverter.ConvertBack(bssType));
			if (!result)
				tcs.SetResult(false);
			...
			using (cancellationToken.Register(() => tcs.TrySetCanceled()))
			{
				...
				return (completedTask == connectTask) && connectTask is { IsCompleted: true, Result: true };
			}
		}
		...
		internal static async Task<bool> DisconnectNetworkAsync(Base.WlanNotificationClient client, Guid interfaceId, TimeSpan timeout, CancellationToken cancellationToken)
		{
			...
			using (cancellationToken.Register(() => tcs.TrySetCanceled()))
			{
				...
				return (completedTask == disconnectTask) && disconnectTask is { IsCompleted: true, Result: true };
			}
		}
		...
		private static IEnumerable<RadioStateSet> EnumerateRadioStates(Base.WlanClient client, Guid interfaceId)
		{
			...
			var dot11PhyTypes = capability.dot11PhyTypes
